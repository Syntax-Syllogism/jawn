...
import UserRestore from '../../../../src/commands/jawn/user/restore.js';
import UserSnapshot from '../../../../src/commands/jawn/user/snapshot.js';
...
describe('jawn user lifecycle commands', () => {
  ...
  it('snapshots assignment state with portable names and excludes profile-owned permission sets', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'jawn-snapshot-test-'));
  ...
  it('restores only missing snapshot assignments and skips DML in dry-run', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'jawn-restore-test-'));
  ...
  it('writes a strip snapshot during dry-run without removal DML', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'jawn-strip-snapshot-test-'));
    const snapshotPath = join(dir, 'snapshot.json');
    const conn = createConnection();
    conn.query.callsFake(async (soql: string) => {
  ...
  it('does not write a strip snapshot when confirmation is declined', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'jawn-strip-snapshot-declined-test-'));
    const snapshotPath = join(dir, 'snapshot.json');
    const conn = createConnection();
    conn.query.callsFake(async (soql: string) => {
  ...
  it('round-trips snapshot output into restore dry-run planning', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'jawn-roundtrip-test-'));
    const snapshotPath = join(dir, 'snapshot.json');
    const conn = createConnection();
    conn.query.callsFake(async (soql: string) => {
  ...
  it('warns and skips missing restore references while continuing the batch', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'jawn-restore-missing-ref-test-'));
    const snapshotPath = join(dir, 'snapshot.json');
    writeFileSync(
      snapshotPath,
      JSON.stringify({
