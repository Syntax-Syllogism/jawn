...
const messages = Messages.loadMessages('@syntax-syllogism/jawn', 'jawn.aep.generate.selector');

export default class JawnAepGenerateSelector extends SfCommand<AepCommandResult> {
  ...
  public async run(): Promise<AepCommandResult> {
    const { flags } = await this.parse(JawnAepGenerateSelector);
