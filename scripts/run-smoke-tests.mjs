...
function inferCheck(command) {
  if (command.includes('user provision')) return 'Review planned users, profiles, assignments, warnings, and changes.';
  if (command.includes('user diff')) return 'Review the reported profile, role, and assignment differences.';
  if (command.includes('user snapshot')) return 'Confirm the snapshot contains the intended existing users and assignments.';
  if (command.includes('user restore')) return 'Confirm the snapshot resolves and the restore plan is additive.';
  if (command.includes('user freeze')) return 'Confirm only login state changes are planned.';
  if (command.includes('user unfreeze')) return 'Confirm only login restoration is planned.';
  if (command.includes('user strip')) return 'Confirm the snapshot is written and no destructive DML occurs in dry-run mode.';
  if (command.includes('user access')) return 'Confirm the target resolves and the structured access result is valid.';
  if (command.includes('aep')) return 'Inspect generated files and confirm the requested template and metadata.';
